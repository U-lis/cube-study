...
| **기준공식 브라우저** | 기준을 데이터가 정한 학습 순서로. 기준별 케이스 목록. 기준별 암기 진도·"외운거 안보기" |
| **퀴즈** | direct(무브 직접 입력) 또는 setup(셋업 + 기준공식 선택). 큐브 시뮬레이터가 채점. "암기한 것만" 필터 |

기준공식의 개수·이름·순서·방향은 전부 데이터가 정한다 (현재 5개: `KG IV BU TC KS`). 코드에 박아두지 않으므로 데이터를 교체하면 화면이 따라간다 — `data/schema-history/README.md` 참고.
