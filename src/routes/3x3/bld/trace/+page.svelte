...
<style>
	...
	.cube-slot {
		 ...
		 * 짧아져도 그만큼 줄지 않기 때문이다. 실측으로 그 고정분이 약 32rem 이다 —
		 * 상단 바 49px 을 포함한 값이고, 시작 전(설정)과 진행 중(패드) 중 **큰 쪽**이다.
		 *
		 * 28rem 이었다가 32rem 으로 늘었다. 화면 위에 되돌아가기(44px)와 화면
		 * 이름(약 2.2rem)이 새로 섰기 때문이다 (FR-NAV-7·9). 고정분을 안 늘리면
		 * 큐브가 그만큼 화면 밖으로 밀려난다 — 넘치는 쪽이 아니라 양보하는 쪽이
		 * 큐브여야 한다 (FR-NAV-19).
		 *
		 ...
		 */
		max-width: clamp(180px, calc(100vh - 32rem), 420px);
		max-width: clamp(180px, calc(100svh - 32rem), 420px);
		margin: 0 auto;
	}
	.cube-gone {
		display: flex;
